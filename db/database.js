...
class liteDb {
  ...
  createTables() {
    ...
    let createMessages =
      "create table if not exists messages (userid text not null, channelid text not null, datetime text not null, message text not null, foreign key (userid) references users(userid), foreign key (channelid) references channels(channelid))";
    //
